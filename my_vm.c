...
static int PT_BITS, PD_BITS, OFFSET_BITS, TAG_BITS;
...
void set_physical_mem() {
    ...
    /* Set aside two pages for pd and clear pd */
    pd = find_next_page();
    find_next_page();
    memset(pd, 0, (int)pow(2, PD_BITS)*ADDR_SIZE);  // pgdir entry of 0 means no pt initialized yet
